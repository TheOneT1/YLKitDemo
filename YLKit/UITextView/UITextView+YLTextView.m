...
- (void)setPlaceHolderLabel:(NSString *)placeholder {
    ...
    self.placeholderLabel.textAlignment = 0;
    self.placeholderLabel.textColor = [UIColor colorWithHex:0xbfbfbf alpha:1];
    self.placeholderLabel.frame = CGRectMake(2, 8, self.frame.size.width,15);
...
- (void)setWordcountLable:(NSNumber *)limitLength {
     ...
     *  字数限制
     */
    self.wordCountLabel = [[UILabel alloc] initWithFrame:CGRectMake(CGRectGetWidth(self.frame) - 65, CGRectGetHeight(self.frame) - 10, 60, 20)];
    self.wordCountLabel.textAlignment = NSTextAlignmentRight;
    self.wordCountLabel.textColor = [UIColor clearColor];
    ...
    self.wordCountLabel.hidden = self.text.length > 0 ? NO : YES;
